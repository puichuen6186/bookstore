...
Route::post('/dangky','UserController@storeRegister')->name('submit.dangky');
...
Route::get('admin/login', function () {
    if(Auth::check('quantri')) {
        return redirect()->route('dashboard');
    }
    return view('layout.admin.login');
})->name('admin.login');

Route::post('/handle-login', 'Admin\AdminController@login')->name('admin.handle-login');

 Route::get('/','Users\HomeController@index')->name('trangchu');
 Route::middleware(['checkQuanTri'])->group(function () {
    Route::group(['prefix' => 'admin'], function () {
        Route::get('/handle-login', 'Admin\AdminController@logout')->name('admin.handle-logout');
...
Route::group(['prefix' => 'user'], function () {
    ...
    Route::get('/return-vnpay', 'Users\PaymentController@return')->name('giohang.checkthanhtoan');
